...
$moduleversion = (Get-InstalledModule -Name "AzureRM").Version
if(!$moduleversion){
    if(Get-InstalledModule -Name "Az"){ Write-Host "This script is for AzureRM. But you seem to have the Az module installed. Please use the other script" }
}else{
    Write-Verbose "Running Script for AzureRM module version $($moduleversion.ToString())"
}
...
if($nicName2){
    if(!$SubnetNameNic2){Write-Error "No subnet name defined for second NIC"; break;}
...
if(!$useManagedDisks){
    ...
    if($moduleversion.Major -lt 6){ 
        ...
        $vmAvSet = New-AzureRmAvailabilitySet -Name $vmAvSetName -ResourceGroupName $ResourceGroupName -Location $location -Sku Aligned -WarningAction SilentlyContinue -PlatformFaultDomainCount $PlatformFaultDomainCount -PlatformUpdateDomainCount $PlatformUpdateDomainCount
    }
}
...
if($vmProductType -ne "waf"){
    ...
    $lbpip = New-AzureRmPublicIpAddress -ResourceGroupName $ResourceGroupName -Location $location -Name "$($lbName)-FrontendIP" -AllocationMethod Static -Sku $lbSku 
    
    $Frontend = New-AzureRmLoadBalancerFrontendIpConfig -Name "$($lbName)-FrontendIP" -PublicIpAddressId $lbpip.Id
    $Probe = New-AzureRmLoadBalancerProbeConfig -Name "$($lbName)-Probe" -Protocol TCP -Port 65500 -IntervalInSeconds 5 -ProbeCount 3 
	$BackendPool = New-AzureRmLoadBalancerBackendAddressPoolConfig -Name "$($lbName)-BackendPool" 
    $lbRule1 = New-AzureRmLoadBalancerRuleConfig -Name "TINA-TCP" -Protocol Tcp -FrontendPort 691 -BackendPort 691 -FrontendIpConfigurationId $Frontend.Id -BackendAddressPoolId $BackendPool.Id  -ProbeId $Probe.Id
	$lbRule2 = New-AzureRmLoadBalancerRuleConfig -Name "TINA-UDP" -Protocol Udp -FrontendPort 691 -BackendPort 691 -FrontendIpConfigurationId $Frontend.Id -BackendAddressPoolId $BackendPool.Id  -ProbeId $Probe.Id
	$lbRule3 = New-AzureRmLoadBalancerRuleConfig -Name "IPSEC-500" -Protocol Udp -FrontendPort 500 -BackendPort 500 -FrontendIpConfigurationId $Frontend.Id -BackendAddressPoolId $BackendPool.Id  -ProbeId $Probe.Id
	$lbRule4 = New-AzureRmLoadBalancerRuleConfig -Name "IPSEC-4500" -Protocol Udp -FrontendPort 4500 -BackendPort 4500 -FrontendIpConfigurationId $Frontend.Id -BackendAddressPoolId $BackendPool.Id  -ProbeId $Probe.Id
	
    #Creates the LBs
    $elb = New-AzureRmLoadBalancer -Name $lbName -ResourceGroupName $ResourceGroupName -Location $location -Sku $lbSku -FrontendIpConfiguration $Frontend -BackendAddressPool $BackendPool -Probe $Probe -LoadBalancingRule $lbRule1,$lbRule2,$lbRule3,$lbRule4
    if($lbSku -eq "Standard"){
        $intBackendPool = New-AzureRmLoadBalancerBackendAddressPoolConfig -Name "$($ilbName)-BackendPool"
        $Frontend = New-AzureRmLoadBalancerFrontendIpConfig -Name "$($ilbName)-FrontendIP" -SubnetId $SubnetId -PrivateIpAddress $ilbIP
	    $lbRule5 = New-AzureRmLoadBalancerRuleConfig -Name "HAPortsRule" -Protocol All -FrontendPort 0 -BackendPort 0 -FrontendIpConfigurationId $Frontend.Id -BackendAddressPoolId $intBackendPool.Id  -ProbeId $Probe.Id
        $ilb = New-AzureRmLoadBalancer -Name "$($ilbName)" -ResourceGroupName $ResourceGroupName -Location $location -Sku $lbSku -FrontendIpConfiguration $Frontend -BackendAddressPool $intBackendPool -Probe $Probe -LoadBalancingRule $lbRule5 
    }
}else{
...
#Creates NSG's for the CGF that allow the firewall to manage what inbound and outbound traffic is accepted Any Any is used as otherwise protocols like ICMP cannot pass
    $rule1 = New-AzureRmNetworkSecurityRuleConfig -Name "Block_Inbound_SSH" -Protocol Tcp -SourcePortRange * -DestinationPortRange 22 -SourceAddressPrefix * -DestinationAddressPrefix * -Access Deny -Priority 100 -Direction Inbound
    $rule2 = New-AzureRmNetworkSecurityRuleConfig -Name "Allow_All_Inbound" -Protocol * -SourcePortRange * -DestinationPortRange * -SourceAddressPrefix * -DestinationAddressPrefix * -Access Allow -Priority 110 -Direction Inbound
    $rule3 = New-AzureRmNetworkSecurityRuleConfig -Name "Allow_All_Outbound" -Protocol * -SourcePortRange * -DestinationPortRange * -SourceAddressPrefix * -DestinationAddressPrefix * -Access Allow -Priority 100 -Direction Outbound
    $nsg = New-AzureRmNetworkSecurityGroup -Name "$vmName-NSG" -ResourceGroupName $ResourceGroupName -Location $location -SecurityRules $rule1,$rule2,$rule3
...
#Creates NSG's for the WAF that limit inbound access to the default HTTP and HTTPS ports plus management access.
    ...
    if ($nic1InternalIP -eq '')
    {
	    if($vmProductType -eq "waf"){
            ...
            if(!$nicname2){
                if($lbSku -eq "Standard"){
		            $nic = New-AzureRmNetworkInterface -ResourceGroupName $ResourceGroupName -Location $location -Name $nicName -PublicIpAddressId $pip.Id -SubnetId $SubnetId -EnableIPForwarding -NetworkSecurityGroupId $nsg.id -LoadBalancerBackendAddressPoolId $BackendPool.Id,$intBackendPool.Id
                }else{
                    $nic = New-AzureRmNetworkInterface -ResourceGroupName $ResourceGroupName -Location $location -Name $nicName -PublicIpAddressId $pip.Id -SubnetId $SubnetId -EnableIPForwarding -NetworkSecurityGroupId $nsg.id -LoadBalancerBackendAddressPoolId $BackendPool.Id
                }
	        }else{
                if($lbSku -eq "Standard"){
		            $nic = New-AzureRmNetworkInterface -ResourceGroupName $ResourceGroupName -Location $location -Name $nicName -PublicIpAddressId $pip.Id -SubnetId $SubnetId -EnableIPForwarding -NetworkSecurityGroupId $nsg.id -LoadBalancerBackendAddressPoolId $BackendPool.Id
    ...
    if($nicName2){
        if ($nic2InternalIP -eq ''){
            if($lbSku -eq "Standard"){
                $nic2 = New-AzureRmNetworkInterface -ResourceGroupName $ResourceGroupName -Location $location -Name $nicName2 -SubnetId $SubnetIdNic2  -EnableIPForwarding -LoadBalancerBackendAddressPoolId $intBackendPool.Id
             }else{
            ...
            if($lbSku -eq "Standard"){
	            $nic2 = New-AzureRmNetworkInterface -ResourceGroupName $ResourceGroupName -Location $location -Name $nicName2 -SubnetId $SubnetIdNic2 -EnableIPForwarding -PrivateIpAddress $nic2InternalIP -LoadBalancerBackendAddressPoolId $intBackendPool.Id
            }else{
