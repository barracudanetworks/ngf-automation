...
$lbSku = "Basic"
#ilbName and ibbIP only used when SKU is Standard
$ilbName = '{0}-ILB' -f $vmSuffix
#This can be left blank to auto allocate next available IP
$ilbIP = ""
...
if($vmProductType -ne "waf"){
    ...
    $lbpip = New-AzPublicIpAddress -ResourceGroupName $ResourceGroupName -Location $location -Name "$($lbName)-FrontendIP" -AllocationMethod Static -Sku $lbSku 
    
    $Frontend = New-AzLoadBalancerFrontendIpConfig -Name "$($lbName)-FrontendIP" -PublicIpAddressId $lbpip.Id
    $Probe = New-AzLoadBalancerProbeConfig -Name "$($lbName)-Probe" -Protocol TCP -Port 65500 -IntervalInSeconds 5 -ProbeCount 3 
	$BackendPool = New-AzLoadBalancerBackendAddressPoolConfig -Name "$($lbName)-BackendPool" 
    ...
    #Creates the LBs
    $elb = New-AzLoadBalancer -Name $lbName -ResourceGroupName $ResourceGroupName -Location $location -Sku $lbSku -FrontendIpConfiguration $Frontend -BackendAddressPool $BackendPool -Probe $Probe -LoadBalancingRule $lbRule1,$lbRule2,$lbRule3,$lbRule4
    if($lbSku -eq "Standard"){
        $intBackendPool = New-AzLoadBalancerBackendAddressPoolConfig -Name "$($ilbName)-BackendPool"
        $Probe = New-AzLoadBalancerProbeConfig -Name "$($ilbName)-Probe" -Protocol TCP -Port 65500 -IntervalInSeconds 5 -ProbeCount 3
	...
	$Frontend = New-AzLoadBalancerFrontendIpConfig -Name "$($lbName)-FrontendIP" -PublicIpAddressId $pip.Id 
	$Probe = New-AzLoadBalancerProbeConfig -Name "$($lbName)-Probe" -Protocol Http -Port 8000 -RequestPath '/' -IntervalInSeconds 5 -ProbeCount 3 
	$BackendPool = New-AzLoadBalancerBackendAddressPoolConfig -Name "$($lbName)-BackendPool" 
...
#Creates NSG's for the CGF that allow the firewall to manage what inbound and outbound traffic is accepted Any Any is used as otherwise protocols like ICMP cannot pass
    $rule1 = New-AzNetworkSecurityRuleConfig -Name "Block_Inbound_SSH" -Protocol Tcp -SourcePortRange * -DestinationPortRange 22 -SourceAddressPrefix * -DestinationAddressPrefix * -Access Deny -Priority 100 -Direction Inbound
    ...
    $nsg = New-AzNetworkSecurityGroup -Name "$vmName-NSG" -ResourceGroupName $ResourceGroupName -Location $location -SecurityRules $rule1,$rule2,$rule3
...
#Creates NSG's for the WAF that limit inbound access to the default HTTP and HTTPS ports plus management access.
    ...
    $rule1 = New-AzNetworkSecurityRuleConfig -Name "Allow_Inbound_HTTP" -Protocol Tcp -SourcePortRange * -DestinationPortRange 80 -SourceAddressPrefix * -DestinationAddressPrefix "$($destinationAddressPrefix)" -Access Allow -Priority 100 -Direction Inbound
    $rule2 = New-AzNetworkSecurityRuleConfig -Name "Allow_Inbound_HTTPS" -Protocol Tcp -SourcePortRange * -DestinationPortRange 443 -SourceAddressPrefix * -DestinationAddressPrefix "$($destinationAddressPrefix)" -Access Allow -Priority 101 -Direction Inbound
    $rule3 = New-AzNetworkSecurityRuleConfig -Name "Allow_Inbound_Management" -Protocol Tcp -SourcePortRange * -DestinationPortRange 8000 -SourceAddressPrefix * -DestinationAddressPrefix "$($destinationAddressPrefix)" -Access Allow -Priority 102 -Direction Inbound
    $rule4 = New-AzNetworkSecurityRuleConfig -Name "Allow_Inbound_SecureManagement" -Protocol Tcp -SourcePortRange * -DestinationPortRange 8443 -SourceAddressPrefix * -DestinationAddressPrefix "$($destinationAddressPrefix)" -Access Allow -Priority 103 -Direction Inbound
    $nsg = New-AzNetworkSecurityGroup -Name "$vmName-NSG" -ResourceGroupName $ResourceGroupName -Location $location -SecurityRules $rule1,$rule2,$rule3,$rule4
    ...
    if ($nic1InternalIP -eq '')
    {
	    if($vmProductType -eq "waf"){
            ...
            if(!$nicname2){
                if($lbSku -eq "Standard"){
		            $nic = New-AzNetworkInterface -ResourceGroupName $ResourceGroupName -Location $location -Name $nicName -PublicIpAddressId $pip.Id -SubnetId $SubnetId -EnableIPForwarding -NetworkSecurityGroupId $nsg.id -LoadBalancerBackendAddressPoolId $BackendPool.Id,$intBackendPool.Id
                }else{
                    $nic = New-AzNetworkInterface -ResourceGroupName $ResourceGroupName -Location $location -Name $nicName -PublicIpAddressId $pip.Id -SubnetId $SubnetId -EnableIPForwarding -NetworkSecurityGroupId $nsg.id -LoadBalancerBackendAddressPoolId $BackendPool.Id
                }
	        }else{
                if($lbSku -eq "Standard"){
		            $nic = New-AzNetworkInterface -ResourceGroupName $ResourceGroupName -Location $location -Name $nicName -PublicIpAddressId $pip.Id -SubnetId $SubnetId -EnableIPForwarding -NetworkSecurityGroupId $nsg.id -LoadBalancerBackendAddressPoolId $BackendPool.Id
    ...
    else
    {
	    ...
	    if($vmProductType -eq "waf"){
		    #When deploying a WAF adds the interface into the Load Balancers Backend pool and NAT Rules
		    $nic = New-AzNetworkInterface -ResourceGroupName $ResourceGroupName -Location $location -Name $nicName -PrivateIpAddress $nic1InternalIP -SubnetId $SubnetId -LoadBalancerBackendAddressPoolId $BackendPool.Id -LoadBalancerInboundNatRuleId $natRule1.Id, $natRule2.Id -NetworkSecurityGroupId $nsg.id
	    }else{
		    $nic = New-AzNetworkInterface -ResourceGroupName $ResourceGroupName -Location $location -Name $nicName -PrivateIpAddress $nic1InternalIP -PublicIpAddressId $pip.Id -SubnetId $SubnetId -EnableIPForwarding -NetworkSecurityGroupId $nsg.id 
	    }
    ...
    if($nicName2){
        if ($nic2InternalIP -eq ''){
            if($lbSku -eq "Standard"){
                $nic2 = New-AzNetworkInterface -ResourceGroupName $ResourceGroupName -Location $location -Name $nicName2 -SubnetId $SubnetIdNic2  -EnableIPForwarding -LoadBalancerBackendAddressPoolId $intBackendPool.Id
             }else{
                $nic2 = New-AzNetworkInterface -ResourceGroupName $ResourceGroupName -Location $location -Name $nicName2 -SubnetId $SubnetIdNic2  -EnableIPForwarding
             }
        }else{
            #Adds to the Internal LB Backend pool if using Standard LBs
            if($lbSku -eq "Standard"){
	            $nic2 = New-AzNetworkInterface -ResourceGroupName $ResourceGroupName -Location $location -Name $nicName2 -SubnetId $SubnetIdNic2 -EnableIPForwarding -PrivateIpAddress $nic2InternalIP -LoadBalancerBackendAddressPoolId $intBackendPool.Id
            }else{
                $nic2 = New-AzNetworkInterface -ResourceGroupName $ResourceGroupName -Location $location -Name $nicName2 -SubnetId $SubnetIdNic2 -EnableIPForwarding -PrivateIpAddress $nic2InternalIP
            }
        }
    }
    ...
    if($useManagedDisks -and $vmProductType -ne "waf" -and $datadiskName1){
      # add the datadisks
        ...
        $vm = Add-AzVMDataDisk -VM $vm -Name $datadiskName1 -DiskSizeInGB $datadisksize -CreateOption Empty -Lun 1
        $vm = Add-AzVMDataDisk -VM $vm -Name $datadiskName2 -DiskSizeInGB $datadisksize -CreateOption Empty -Lun 2
        $vm = Add-AzVMDataDisk -VM $vm -Name $datadiskName3 -DiskSizeInGB $datadisksize -CreateOption Empty -Lun 3
    }elseif($vmProductType -ne "waf" -and $datadiskName1){
        # add the datadisks
        Write-Verbose 'Adding data disks'
        $vm = Add-AzVMDataDisk -VM $vm -Name $datadiskName1 -DiskSizeInGB $datadisksize -CreateOption Empty -Lun 1 -VhdUri $dataDiskUri1
        $vm = Add-AzVMDataDisk -VM $vm -Name $datadiskName2 -DiskSizeInGB $datadisksize -CreateOption Empty -Lun 2 -VhdUri $dataDiskUri2
        $vm = Add-AzVMDataDisk -VM $vm -Name $datadiskName3 -DiskSizeInGB $datadisksize -CreateOption Empty -Lun 3 -VhdUri $dataDiskUri3
    }
