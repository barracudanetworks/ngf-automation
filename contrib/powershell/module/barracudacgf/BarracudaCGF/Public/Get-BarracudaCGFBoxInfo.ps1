...
[switch]$notHTTPs
	...
	try{
        if($creds){
		    $results =Invoke-WebRequest -Uri "http$($s)://$($deviceName):$($devicePort)/rest/control/v1/box/$($subject)" -Method GET -Headers $header -Credential $creds -UseBasicParsing -SkipCertificateCheck
        }else{
            $header = @{"X-API-Token" = "$token"}
            $results =Invoke-WebRequest -Uri "http$($s)://$($deviceName):$($devicePort)/rest/control/v1/box/$($subject)" -Method GET -Headers $header -UseBasicParsing -SkipCertificateCheck
        }
	}catch{
