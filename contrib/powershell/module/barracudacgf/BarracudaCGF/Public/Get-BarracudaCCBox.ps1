...
[switch]$notHTTPs
    ...
    try{
           	$results =Invoke-WebRequest -Uri "http$($s)://$($deviceName):$($devicePort)/rest/cc/v1/ranges/$($range)/clusters/$($cluster)/boxes/$($box)$($getextra)" -Method GET -Headers $header -UseBasicParsing -SkipCertificateCheck
