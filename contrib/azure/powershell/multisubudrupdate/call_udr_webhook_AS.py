...
def call_webhook(url, subid, id, boxip, haip,viewstate,viewstategen,eventvalidation):
	...
	payload = {
        ...
        'webhookdata' : '[{"SubscriptionId":"' + str(subid) + '","id":"' + str(id) + '","properties":{"OldNextHopIP":"' + str(haip) + '","NewNextHopIP":"' + str(boxip) + '"}}]'
        }
	...
	try:
		request.add_data(urllib.urlencode(payload))
...
def main():
	...
	#hardcode view state
	viewstate = 'WwSDVBwDRC3F5xPJD1As+c6NQBtkP/trcimJ1P/RkMZwzlQQC+qyQFudr80C1eBDaY4Qm+vZFfeiJIk3LmXVDJa9Nm9tdSD/gY03q6eUDzY='
	viewstategen = 'CA0B0334'
	eventvalidation = 'lI1mGaY9o+xFlblKnXitYobk/jlNhhG1Dhfk9s6NZXIY7oGi1XXbt1vYlzxxSNSnsWUcM0Nz2A6FNUxO275gWiaH25A50gdR2wfy9vh42+SqQwHClggvXplu/0aefyXrt1cDGE7pqnncZ+iU2pOZlA=='
	...
	while condition:
		...
		if(commands.getoutput('ps -fC ' + servicename).find(servicename) != -1):
			...
			webhook = call_webhook(options.webhookurl, str(subid)[2:-2], vnetname, boxip, haip, viewstate,viewstategen,eventvalidation)
			...
			if(options.down == "YES"):
				...
				webhook = call_webhook(options.webhookurl, str(subid)[2:-2], vnetname, haip, boxip,viewstate,viewstategen,eventvalidation)
				if (webhook != 'FAILED'):
					condition = False
